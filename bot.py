...
class PlanGenerator:
    ...
    DRILLS = {
        ...
        "coach_development": [
            {
                "name": "תצפית מודרכת",
                "desc": "המאמן עוצר את התרגיל ושואל שאלות במקום לתת הוראות.",
                "setup": "כל תרגיל",
                "points": ["מה ראית?", "למה בחרת בזה?", "מה היה האפשרות האחרת?"],
                "animation": "עצירה → שאלה → תשובת שחקן → המשך",
            },
        ],
    }
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.error("save_user failed: %s", e)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "new_weekly":
        ...
        text = (
            ...
            "רוצה לשנות? התחל מערך חדש או שלח /start"
        )
        ...
        text = (
            ...
            "הבוט מותאם ל-11 שחקנים ו-90 דקות, עם דגש חזק על חשיבה וסריקה."
        )
        await query.edit_message_text(text, parse_mode="Markdown")
...
async def receive_focus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "למשל: עבודה על שחקן מסוים, כושר, מסירות...\n\n"
...
def format_plan(plan: Dict) -> str:
    """מעצב את המערך להודעה יפה בטלגרם"""
...
async def unknown_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "כדי להתחיל לחץ על הכפתורים למטה, או שלח /start",
    )
